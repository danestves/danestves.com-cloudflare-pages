    ...
    {blogs
      ? blogs.map(blog => (
          <Fade key={blog.id}>
            <Link
              to={`/blog/${blog.slug}`}
              className="flex flex-wrap items-stretch my-5 overflow-hidden bg-white border border-transparent rounded-lg shadow hover:border-indigo-900 dark:hover:border-white dark:bg-indigo-900 hover:shadow-lg dark:shadow-white dark:hover:shadow-white-lg transition-all duration-200"
            >
              <Img
                fluid={blog.cover.childImageSharp.fluid}
                className="w-full sm:w-2/5 lg:w-1/3 xl:w-1/5 transition-all duration-200"
              />

              <div className="w-full px-4 py-5 sm:w-3/5 lg:w-2/3 xl:w-4/5 transition-all duration-200">

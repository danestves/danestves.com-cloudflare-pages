...
cover: '/static/images/how-to-create-awesome-ogimages-using-react-components/banner.jpeg'
seo:
  {
    title: 'Qué es Flyyer y cómo crear impresionantes og:images con React',
    description:
...
Tengo ya un tiempo probando [Flyyer](https://flyyer.io) el cual es un _`og:image` as a service_, lo que nos promete este servicio es crear `templates` con React o Vue y usarlos para crear imágenes con base en ellos, en estos `templates` podemos pasarle variables con los que podamos construir nuestras imágenes, por ejemplo el `title` y el `backgroundImg` de un post y podemos obtener los siguientes resultados:

<a
  aria-label="Flyyer.io | Flyyer.com platform for better links"
  href="https://flyyer.io"
>
  <Image
    alt="Preview de image con diseño de Twitter"
    className="rounded-xl border border-white overflow-hidden max-w-md"
    height={346}
    src="/static/images/how-to-create-awesome-ogimages-using-react-components/img1.png"
    objectFit="fill"
    width={466}
  />
</a>

Inclusive podemos detectar si el `userAgent` es WhatsApp y generar una imagen específica para esta app:
...
>
  <Image
    ...
    height={152}
    src="/static/images/how-to-create-awesome-ogimages-using-react-components/img2.png"
    objectFit="fill"
    width={480}
  />
</a>

En este caso como ejemplo es solo un cuadrado gris, pero podemos meter lo que sea allí; gracias a la manera en como funciona [Flyyer](https://flyyer.io) podemos generar un view distinto dependiendo del `userAgent`, que actualmente son muchísimos los que soporta:
...
Esto son los actuales, pero poco a poco van agregando más agentes para que podamos personalizar aún más nuestras imágenes

## ¿Cómo empezar con Flyyer?

Primero que todo quiero comentarte que este tutorial será para React, pero si quieres saber cómo hacerlo con Vue déjamelo saber en mi [Twitter](https://twitter.com/messages/compose?recipient_id=554765148) con un DM.
...
Se te presentarán algunas opciones para crear la aplicación, para este tutorial seguiremos con la opción de `react-typescript-tailwind`.

Si aún no sabes de TypeScript te recomiendo [este post](/es/posts/how-typescript-enforced-cleaner-architecture-in-react-app) donde enseño TS con React), luego de elegir esa opción le damos enter y esperamos a que se cree la carpeta con la aplicación.

> **Nota:** Antes de que se termine necesitaremos una **FLYYER_KEY** para poder hacer deploy de nuestra aplicación dentro de Flyyer, para eso nos crearemos una cuenta en Flyyer y tomaremos la key de nuestra cuenta, entra a **https://flyyer.io/dashboard/YOUR_USERNAME/settings** para obtener la key. Guarda esta key, ya que la utilizaremos más adelante.
...
Puedes notar que se manda en formato de URL Params, no tenemos que preocuparnos porque los caracteres se vean de una manera distinta, por ejemplo los espacios son transpilados a `%20` ya Flyyer se encarga de parsear estos datos y entregárnoslo de una manera limpia en el componente de React.
...
<Image
  ...
  height={342}
  src="/static/images/how-to-create-awesome-ogimages-using-react-components/img3.png"
  width={607}
/>
...
<Image
  alt="Flyyer Studio"
  height={1001}
  src="/static/images/how-to-create-awesome-ogimages-using-react-components/img4.png"
  width={1156}
/>
...
<Image
  alt="Flyyer Studio"
  height={1001}
  src="/static/images/how-to-create-awesome-ogimages-using-react-components/img5.png"
  width={1156}
/>
...
<Image
  ...
  height={1001}
  src="/static/images/how-to-create-awesome-ogimages-using-react-components/img6.png"
  width={1156}
/>
...
Como puedes ver cada vez salen más y más herramientas que nos hacen el desarrollo muchísimo más fácil cada día, ahora solo llamamos a una librería y ya hace todo el trabajo por nosotros. **Tercerizar** los servicios es para mi una de las mejores cosas de la programación.

Espero que este post haya sido de mucha ayuda y si quieres que haga un post sobre cómo realizar las `og:image` con Vue déjamelo saber en un comentario en Twitter y así sabré cuantas personas están interesadas por el siguiente post 🚀

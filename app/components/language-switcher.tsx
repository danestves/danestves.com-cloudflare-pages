...
const LanguageSwitcher = (): JSX.Element => {
  ...
  return (
    <Menu as="div" className="inline-flex relative text-left">
      <Menu.Button className="inline-flex items-center space-x-4 rounded-full focus:outline-none focus:ring-4 focus:ring-primary/50 md:py-2 md:px-4 md:rounded-md">
        <div className="w-9 h-9 rounded-full md:w-6 md:h-6">
          <Flag countryCode={rootData?.country} />
        </div>
        <span className="font-semibold text-primary uppercase sr-only md:not-sr-only">
      ...
      >
        <Menu.Items className="overflow-hidden absolute top-full right-0 mt-2 w-max bg-white dark:bg-[#292929] rounded-md focus:outline-none ring-1 ring-black shadow-lg origin-top-right ring-opacity-5">
          <Menu.Item
            as="button"
            className="group inline-flex items-center p-4 mx-auto space-x-4 hover:bg-primary transition-colors duration-200"
            onClick={handleChangeLanguage}
          >
            <div className="w-6 h-6 rounded-full">
              <Flag countryCode={rootData?.country} locale={locale} />
            </div>
            <span className="font-semibold text-primary group-hover:text-white uppercase transition-colors duration-200">
